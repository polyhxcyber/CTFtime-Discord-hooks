...
def build_message(max_ctfs: int, days: int) -> Union[Hook, None]:
    ...
    message = os.getenv("MESSAGE") or f"CTFs annoncés pour les prochains {days} jours"
    avatar_url = os.getenv("AVATAR_URL") or DEFAULT_ICON
    username = os.getenv("USERNAME") or "CTFTime"
    return Hook(
        username=username,
        content=message,
        embeds=embeds,
...
def handler(event=None, context=None):
    ...
    max_ctfs = int(os.getenv("MAX_CTFS")) or 10
